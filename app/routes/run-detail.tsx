...
export default function RunDetail() {
    ...
    useEffect(() => {
        ...
        s.on("message", onMsg);
        return () => { s.off("message", onMsg); s.emit("leave", { runId }); };
    }, [runId]);
                ...
                <div className="max-h-[60vh] overflow-auto text-sm space-y-1">
                    {logs.map(l => (
                        <div key={l.id} className="grid grid-cols-[140px,100px,1fr] gap-2">
                            <div className="text-neutral-500">{new Date(l.ts).toLocaleTimeString()}</div>
                            <div className="text-neutral-700">{l.nodeId ?? "-"}</div>
                            <div><span className="mr-2">{l.level}</span>{l.message}</div>
                        </div>
                    ))}
                    {logs.length === 0 && <div className="text-neutral-500">No logs yet…</div>}
                </div>
            </div>
        </div>
    );
